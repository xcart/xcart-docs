...
3.  The package is an archive, so unpack it to the web-root directory of your server. You can do it using the archive tools of your Control panel or using the following command via SSH: 
    ...
    ```php
    unzip x-cart-5.4.0.10-en.zip
    ```

    for tgz archive: 

    ```php
    tar -xzpf x-cart-5.4.0.10-en.tgz
    ```
...
Also, if you change the cache driver setting on the server do not forget to add a cron job described [here](https://kb.x-cart.com/general_setup/installation/server/choosing_data_cache_driver.html#file-system-cache "Installation Guide").
