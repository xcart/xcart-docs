...
* **Downloads limit (per 1 ordered item)**: Secify the number of product download attempts that should be available to the buyer who has purchased the product (i.e. how many times the buyer of a downloadable product should be able to download the product using the download link provided to them). To allow unlimited access, set this value to zero. **Important**: If using Amazon Signed URLs, this settings controls the limit of link renewal, because it is impossible to control downloads on the Signed URL.
