...
In the "_Warehouse_" mode, the products all come from different owners (vendors), but a customer buying products from different vendors still gets a single order, and all the products are shipped to them from a single warehouse location. When running a multivendor store in this mode, different businesses will have their own preferences as to who should handle communication with the customer at each stage of the order fulfillment process. The addon Messages can adapt to the needs of different businesses as it provides a setting that enables the store administrator to specify who should handle the communication with the customers - **Allow direct communication between vendor and buyer in the "Warehouse" multivendor mode**: 
   ![msgs_settings1.png]({{site.baseurl}}/attachments/ref_4Ox4FPDB/msgs_settings1.png)
If the administrator chooses to handle all the communication with the customers themselves, they should keep the above said setting disabled. In this case all order related messages from customers will be addressed to the administrator. If, however, the administrator wishes to enable the customers to choose by themselves who they want to contact - the individual vendors or the store administrator - this setting needs to be enabled. In this case the addressee of each message will be chosen by the customer.
