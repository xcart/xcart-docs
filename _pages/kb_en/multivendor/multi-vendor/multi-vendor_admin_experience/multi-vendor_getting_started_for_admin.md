...
Note that in the "Vendors as separate shops" multivendor mode, the store administrator can limit the set of US states for which a request for tax rates may be sent by the store. This can be done using the setting "Calculate taxes for US in designated states only" on the settings page of the tax service integration addon (AvaTax or TaxJar) and in the Company details section of the vendor profile.

## Step 12: Adjust the vendor payout options
Unless the multivendor store/online marketplace has been configured to use a payment method that will automatically transfer the money from the sales to the accounts of participating vendors, the store owner will have to track vendor earnings and to do payouts into the accounts of the vendors. To facilitate the process, payout requests can be used. This feature enables vendors to request a payout of their account balance, and the administrator - to approve or decline such a request. If the store administrator intends to use this feature, they should adjust the payout related settings in the Finance section of the {% link "Multi-vendor addon settings page" ref_nFq48dhr %}. For example, the administrator can set a minimal balance that a vendor must have to be able to request a payout of their earnings, enable an email notification for payout requests, and so on.
