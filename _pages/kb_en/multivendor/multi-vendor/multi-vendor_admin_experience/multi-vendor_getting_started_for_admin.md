...
Note that in the "Vendors as separate shops" multivendor mode, the store administrator can limit the set of US states for which a request for tax rates may be sent by the store. This can be done using the setting "Calculate taxes for US in designated states only" on the settings page of the tax service integration addon and in the Company details section of the vendor profile.
