...
{% toc Рассмотрим настройки обмена данными в магазине X-Cart и в системе "1С:Предприятие"%}
...
Откроется страница параметров модуля **1C integration**

![001.jpg]({{site.baseurl}}/attachments/ref_1yLiVv5G/001.jpg)

- _**Адрес сайта**_ - Ссылка на магазин понадобится для настройки синхронизации в системе "1С:предприятие".
- _**Режим загрузки категорий из 1С**_ - **Добавление**: в магазине будут созданы новые категории. **Замещение**: категории магазина будут замещены категориями, загруженными из "1С:предприятие".
- _**Режим загрузки продуктов из 1С**_ - **Добавление**: в магазине будут созданы новые товары. **Замещение**: товары магазина будут замещены товарами, загруженными из "1С:предприятие".
- _**Импортировать значения реквизитов**_ - В "1С:предприятие" необходимо создать реквизиты (дата, номер заказа и т.д.) для настройки синхронизации товаров. Значения реквизитов будут перенесены в магазин в виде атрибутов товаров.
- _**Название импортируемого типа цены**_ - Если в системе "1С:предприятие" настроено несколько типов цен, в этом поле укажите тип цен, который будет загружен из "1С:предприятие" в магазин.
- _**Режим импортирования картинок продукта**_ - **Не импортировать катринки**: товары будут выгружены из "1С:предприятие" без изображений; **Добавлять к имеющимся картинкам**: изображения товаров из "1С:предприятие" будут добавлены к изображениям товара в X-Cart; **Замещать имеющиеся картинки**: изображения товаров в X-Cart будут заменены на изображения товаров из "1С:предприятие".
- _**Префикс файла с информацией о товарах**_ - Название файла, загружаемого из "1С:предприятие". Значение по умолчанию: import. Значение поля следует изменить, если "1С:предприятие" присылает файл с другим названием.
- _**Префикс файла с информацией о ценах**_ - Название файла, загружаемого из "1С:предприятие". Значение по умолчанию: offers. Значение поля следует изменить, если "1С:предприятие" присылает файл с другим названием.
...
На открывшейся странице со списком типовых соглашений нажмите **Создать**. Заполните поля на вкладке **Основное**. Поле **Сегмент клиентов** оставьте пустым (в некоторых конфигурациях ситемы "1С:Предприятие" оно может называться **Сегмент партнёров**).
...
{% note info %}
В других конфигурациях системы "1С:Предприятие" набор полей может отличаться от представленного в примере. 
{% endnote %}
...
Затем выберите параметр **Соглашение**, задайте для него вид сравнения **равно,** в качестве значения выберите из списка **типовое соглашение**, которое создали специально для выгрузки товаров на сайт (см. пункт [Типовое соглашение](#типовое-соглашение)).
...
<table class="ui celled padded compact small table">
  <tbody>
  	<tr>
      <td class="confluenceTd">Остаток на складе</td>
      <td class="confluenceTd">Больше</td>
      <td class="confluenceTd">0,001</td>
  	</tr>
  	<tr>
      <td class="confluenceTd">Цена за единицу</td>
      <td class="confluenceTd">Больше</td>
      <td class="confluenceTd">0,01</td>
  	</tr>
  	<tr>
      <td class="confluenceTd">Соглашение</td>
      <td class="confluenceTd">Равно</td>
      <td class="confluenceTd">Интернет-магазин (розница)</td>
  	</tr>
  </tbody>
</table>
