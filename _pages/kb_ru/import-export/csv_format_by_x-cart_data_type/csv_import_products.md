...
<table class="ui compact padded celled small table">
  <tbody>
    ...
    <tr>
      ...
      <td colspan="1" class="confluenceTd">
        <p>Определяет, а какие категории входит товар. Категории разделяются символами &gt;&gt;&gt; .</p>
...
При импорте вариантов товаров, значения полей, не связанных с вариантами (т.е. всех полей кроме тех, которые добавлены модулем Product Variants), не должны повторяться для каждого варианта. Это значит, что такие поля, как memberships, productClass, taxClass, enabled, shippable, categories, inventoryTracking, name, description и тп, нужно заполнять только один раз для набора вариантов товара. Если указать все эти значения для каждого артикула варианта, значения могут дублироваться или перезаписываться последующими значениями. 
