...
published: true
---
Администратор - владелец магазина или его представитель, который в X-Cart представлен как пользователь с полными правами на управление магазином. Он отвечает за настройку, запуск и поддержание функционирования магазина.

{% toc %}

# Настройка и запуск магазина

Вы можете убедиться, что используете магазин X-Cart с функциональностью **Multivendor**, проверив секцию "**My addons**" в интерфейсе администратора: в версии с поддержкой **Multivendor** модуль **Multi-vendor**, составляющий ее ядро, будет установлен и включен. Фактически, из-за особенностей архитектуры данный модуль не может быть отключен, поэтому он всегда будет включен, пока установлен. Единственный способ отключить функциональность **Multivendor** в магазине - это удалить модуль **Multi-vendor** полностью. 

Модуль **Multi-vendor** требует настройки перед началом использования. Далее приводится информация о необходимых настройках.
